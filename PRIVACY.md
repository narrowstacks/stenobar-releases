...
| Microsoft MAI-Transcribe | `<your resource>.cognitiveservices.azure.com` | Audio | https://learn.microsoft.com/en-us/azure/foundry/responsible-ai/speech-service/speech-to-text/data-privacy-security (Microsoft states it does not retain or store data sent to fast transcription: audio is processed in server memory only, nothing is stored at rest, and data in transit is encrypted; with speaker labels on, the voice characteristics used to tell speakers apart are discarded when the request finishes, with no enrollment and no tracking across files; Azure's separate audio-logging feature is off unless you enable it on the resource yourself, and Stenobar never turns it on) |
...
| TypeSafe Jev | `api.typesafe.ai` | Dictated thought text (thoughts routing only: Jev decides the category; the provider you pair with it writes the title and description under its own row above) | https://typesafe.ai/legal/privacy-policy (TypeSafe states it does not train on customer requests or responses) |
